...
if(isset($_POST['submit'])) {
  ...
  $p_name = "Question Plugin";
  $p_version = "1.4.3";
  $p_type = "Question";
  $p_desc = "This plugin gives to your users the ability post questions in your social network.";
  $p_icon = "question";
  $p_db_charset = 'utf8';
  ...
  if( !$resource->num_rows )
  {   $sql = "INSERT INTO `".$sql_db_name."`.`plugins_system`
      ( system_version )
      VALUES
      ( '1.0')"; 
    $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  ...
  //install plugins lang question
  foreach($wo['plugin_question'] as $k => $v){
	 $langz = "";
     $new_k = 'plugin_question_'.str_replace("-",'_',$k);
	 $sql = "SELECT * FROM `Wo_Langs` WHERE lang_key='{$new_k}' LIMIT 1";
     $resource = $sqlConnect->query($sql);
     if(!$resource->num_rows ){
			$sqlConnect->query("INSERT INTO `Wo_Langs` (`lang_key`) VALUES ('{$new_k}')");
  ...
  // CREATE posts_questions_options
  $sql = "SHOW TABLES FROM ".$sql_db_name." LIKE 'posts_questions_options'";
  $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  
  if( !$resource->num_rows )
  {  $sql = "
CREATE TABLE IF NOT EXISTS `posts_questions_options` (
  ...
  // CREATE posts_questions
  $sql = "SHOW TABLES FROM ".$sql_db_name." LIKE 'posts_questions'";
  $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
...
CREATE TABLE IF NOT EXISTS `posts_questions` (
  ...
  $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
   ...
   // CREATE question_votes
  $sql = "SHOW TABLES FROM ".$sql_db_name." LIKE 'questions_options_votes'";
  $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  
  if( !$resource->num_rows )
  {  $sql = "
  CREATE TABLE IF NOT EXISTS `questions_options_votes` (
  ...
  //ADMIN SETTING
    $sql = "SHOW COLUMNS FROM `".$sql_db_name."`.`plugins_system` LIKE 'question_limit_answers'";
  $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  if( !$resource->num_rows )
  {	$sql = " ALTER TABLE plugins_system ADD COLUMN `question_limit_answers` INT NOT NULL DEFAULT 20 ";    
    $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  } 
   
       $sql = "SHOW COLUMNS FROM `".$sql_db_name."`.`plugins_system` LIKE 'question_enable_add_answers'";
  $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  if( !$resource->num_rows )
  {	$sql = " ALTER TABLE plugins_system ADD COLUMN `question_enable_add_answers` INT NOT NULL DEFAULT 1 ";    
    $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  } 
  
      $sql = "SHOW COLUMNS FROM `".$sql_db_name."`.`plugins_system` LIKE 'question_enable_multi_votes'";
  $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  if( !$resource->num_rows )
  {	$sql = " ALTER TABLE plugins_system ADD COLUMN `question_enable_multi_votes` INT NOT NULL DEFAULT 1 ";    
    $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  } 
  
      $sql = "SHOW COLUMNS FROM `".$sql_db_name."`.`plugins_system` LIKE 'question_enable_invite_to_friends'";
  $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  if( !$resource->num_rows )
  {	$sql = " ALTER TABLE plugins_system ADD COLUMN `question_enable_invite_to_friends` INT NOT NULL DEFAULT 1 ";    
    $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  }
  $sql = "SHOW COLUMNS FROM `".$sql_db_name."`.`plugins_system` LIKE 'question_enable_email_invite_to_friends'";
  $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  if( !$resource->num_rows )
  {	$sql = " ALTER TABLE plugins_system ADD COLUMN `question_enable_email_invite_to_friends` INT NOT NULL DEFAULT 1 ";    
    $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  } 
  
      $sql = "SHOW COLUMNS FROM `".$sql_db_name."`.`plugins_system` LIKE 'question_enable_show_in_profile'";
  $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  if( !$resource->num_rows )
  {	$sql = " ALTER TABLE plugins_system ADD COLUMN `question_enable_show_in_profile` INT NOT NULL DEFAULT 1 ";    
    $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  } 
    
	
$sql = "SHOW COLUMNS FROM `".$sql_db_name."`.`Wo_Posts` LIKE 'question_id'";
  $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  if( !$resource->num_rows )
  {	$sql = " ALTER TABLE Wo_Posts ADD COLUMN `question_id` INT NOT NULL DEFAULT 0 ";    
    $resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
  } 	
...
.btn_bt{border:1px solid #CCC; background-color:#CCC; text-align:center; padding:5px;}
 ...
 <div class="container mt20"><div class="row"><div class="col-sm-12">
                ...
                <div class="panel-heading"> Welcome to <strong>Question</strong> installation!</div>
...
Before continuing, please be sure you have reviewed the installation instructions provided in <b>Documentation</b>.				
				...
				<form autocomplete="off" action="install_question.php" method="post">                             
					<?php if($ready == 0){
