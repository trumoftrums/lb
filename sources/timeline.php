...
$ads_position = array();
if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Ads', $wo['plugin_list']['plugin_actived']) ){	
	$ads_positions = str_replace(", ",',',$wo['system']['enable_ad_rigth_place']);
	$ads_position = explode(",", $ads_positions); 
}	
if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Ads', $wo['plugin_list']['plugin_actived'] ) && $wo['system']['pagelet_ego'] == 1 && in_array($type, $ads_position)){

	$wo['content']     = Wo_LoadPage("plugins/{$type}");
} else {
