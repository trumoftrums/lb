...
if ($f == 'login') {
    ...
    if (isset($_POST['username']) && isset($_POST['password'])) {
        ...
        if (empty($errors) && $phone == 0) {
			...
			/* plugin points */
			if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){ update_points( $userid, 'login' ); }	
			/* plugin points */
            $_SESSION['user_id'] = $session;
            setcookie(
                "user_id",
...
if ($f == 'register') {
    ...
    if (empty($_POST['email']) || empty($_POST['username']) || empty($_POST['password']) || empty($_POST['confirm_password'])) {
		...
		if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){
			if(!empty($_POST['signup_invite'])){
				$signup_invite = Wo_Secure($_POST['signup_invite']);
				$invite = $sqlConnect->query("SELECT * FROM invites WHERE invite_code='{$signup_invite}' LIMIT 1");
				if($invite->num_rows == 0) { 
					$errors = $error_icon .'The invite code you have entered is invalid.';
				} else { 
					$invite_exist = 1;
				}
			}else if(!empty($_POST['signup_referer'])){
				$signup_referer = Wo_Secure($_POST['signup_referer']);
				$pp_referer = Wo_UserIdForLogin($signup_referer);
				if(empty($pp_referer)) {
					$errors = $error_icon .'The user referer no exist.';
				} else {
					$referer_exist = 1;
				}
			}
    ...
    if (empty($errors)) {
		...
		/* plugin points */				
		if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){ 
		    $new_user = Wo_UserIdForLogin($_POST['username']);
			if(!empty($new_user) && $invite_exist == 1){
				// INVITE CODE FEATURES
				$invitation = $invite->fetch_assoc();
				update_points($invitation['invite_user_id'], 'invite');
				update_points($new_user, 'signup_with_invite');
				update_points($new_user, 'signup');
				/* add the friend request */
				$sqlConnect->query("INSERT INTO Wo_Followers (following_id, follower_id, active) VALUES ('{$invitation['invite_user_id']}', '{$new_user}', '1')");
				$sqlConnect->query("INSERT INTO Wo_Followers (following_id, follower_id, active) VALUES ('{$new_user}', '{$invitation['invite_user_id']}', '1')");
				// DELETE INVITE CODE
				$sqlConnect->query("DELETE FROM invites WHERE invite_id='{$invitation['invite_id']}' AND invite_user_id ='{$invitation['invite_user_id']}'  LIMIT 1");
			}else if(!empty($pp_referer) && !empty($new_user) && $referer_exist == 1){
				// INVITE CODE FEATURES
			    update_points($pp_referer, 'refer');
				update_points($new_user, 'signup'); 
				// add the friend request
				$sqlConnect->query("INSERT INTO Wo_Followers (following_id, follower_id, active) VALUES ('{$pp_referer}', '{$new_user}', '1')");
				$sqlConnect->query("INSERT INTO Wo_Followers (following_id, follower_id, active) VALUES ('{$new_user}', '{$pp_referer}', '1')");
			}
		}	
		/* plugin points */
            if ($activate == 1) {
        ...
        if (!empty($Userdata['user_id'])) {
            if (isset($_FILES['avatar']['name'])) {
                if (Wo_UploadImage($_FILES["avatar"]["tmp_name"], $_FILES['avatar']['name'], 'avatar', $_FILES['avatar']['type'], $_POST['user_id']) === true) {
                    $Userdata = Wo_UserData($_POST['user_id']);
					/* plugin points */
					if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){ update_points( $wo['user']['user_id'], 'editphoto' ); }
					/* plugin points */	
                }
            }
            if (isset($_FILES['cover']['name'])) {
                if (Wo_UploadImage($_FILES["cover"]["tmp_name"], $_FILES['cover']['name'], 'cover', $_FILES['cover']['type'], $_POST['user_id']) === true) {
                    $Userdata = Wo_UserData($_POST['user_id']);
					/* plugin points */
					if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){ update_points( $wo['user']['user_id'], 'editcover' ); }
					/* plugin points */
                }
            }
            if (empty($errors)) {
    ...
    if (isset($_GET['following_id']) && Wo_CheckMainSession($hash_id) === true) {
        if (Wo_IsFollowing($_GET['following_id'], $wo['user']['user_id']) === true || Wo_IsFollowRequested($_GET['following_id'], $wo['user']['user_id']) === true) {
            ...
            if (Wo_RegisterFollow($_GET['following_id'], $wo['user']['user_id'])) {
				...
				/* plugin points */
				if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){ update_points( $wo['user']['user_id'], 'addfriend' ); }
				/* plugin points */	
            }
        }
    }
        ...
        if (Wo_CheckSession($hash_id) === true) {
            if (!empty($_POST['lang_key'])) {
                ...
                if ($sql_fetch['count'] == 0) {
                    ...
                    if ($mysqli) {
                        $data['status'] = 200;
                        $data['url'] = Wo_SeoLink('index.php?link1=admincp&page=edit_key&id=' . $lang_key);
                    }
                } else {
                    $data['status'] = 400;
    ...
    if ($s == 'load_more_posts') {
        ...
        if (!empty($_GET['filter_by_more']) && !empty($_GET['after_post_id'])) {
            ...
            if (!empty($_GET['posts_count']) && !empty($get_posts) && $is_api == false) {
				...
				/*start format = json*/
		} if(empty($_POST['format']) && $_GET['format'] == 'json'){ header("Content-type: application/json"); echo json_encode($data); }
		/*end format = json*/
        exit();
    }
    if ($s == 'edit_post') {
    ...
    if ($s == 'register_like') {
        if (!empty($_GET['post_id']) && Wo_CheckMainSession($hash_id) === true) {
            if (Wo_AddLikes($_GET['post_id']) == 'unliked') {
				...
				/*plugin*/
				if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){ update_points( $wo['user']['user_id'], 'like' ); }	
				/*plugin*/
            }
            $data['dislike'] = 0;
    ...
    if ($s == 'register_share') {
        if (!empty($_GET['post_id']) && Wo_CheckMainSession($hash_id) === true) {
            if (Wo_AddShare($_GET['post_id']) == 'unshare') {
				...
				/* plugin points */
				if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){ update_points( $wo['user']['user_id'], 'joingroup' ); }
				/* plugin points */
            }
        }
        header("Content-type: application/json");
    ...
    if ($s == 'register_comment') {
        if (!empty($_POST['post_id']) && isset($_POST['text']) && Wo_CheckMainSession($hash_id) === true) {
            ...
            if (!empty($wo['comment'])) {
				...
				/* plugin points */
				if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){ update_points( $wo['user']['user_id'], 'comment' ); }	
				/* plugin points */
            }
        }
        header("Content-type: application/json");
...
if ($f == 'apps') {
    ...
    if ($s == 'acceptPermissions') {
        ...
        if ($acceptPermissions === true) {
            $import = Wo_GenrateCode($wo['user']['user_id'], $_GET['id']);
            $app    = $_GET['url'] . '?code=' . $import;
            $data   = array(
                'status' => 200,
                'location' => $app
            );
...
if ($f == 'pages') {
    if ($s == 'create_page') {
        ...
        if (empty($errors)) {
            ...
            if ($register_page) {
			/* plugin points */
			if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){ update_points( $wo['user']['user_id'], 'newpage' ); }	
			/* plugin points */
                $data = array(
                    'status' => 200,
    ...
    if (isset($_GET['group_id']) && Wo_CheckMainSession($hash_id) === true) {
        if (Wo_IsGroupJoined($_GET['group_id']) === true || Wo_IsJoinRequested($_GET['group_id'], $wo['user']['user_id']) === true) {
            if (Wo_LeaveGroup($_GET['group_id'], $wo['user']['user_id'])) {
				...
				/* plugin points */
				if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){ update_points( $wo['user']['user_id'], 'leavegroup' ); }
				/* plugin points */
            }
        } else {
...
if ($f == 'groups') {
    if ($s == 'create_group') {
        ...
        if (empty($errors)) {
            ...
            if ($register_group) {
			/* plugin points */
			if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){ update_points( $wo['user']['user_id'], 'newgroup' ); }	
			/* plugin points */
                $data = array(
                    'status' => 200,
...
if ($f == 'invite_user') {
    ...
    if (empty($errors)) {
        ...
        if ($send) {
			...
			/* plugin points 
			if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){ update_points( $wo['user']['user_id'], 'invite' ); }*/	
			/* plugin points */
        } else {
            $errors[] = $error_icon . $wo['lang']['processing_error'];
        }
    ...
    if (!empty($_GET['id']) && Wo_CheckMainSession($hash_id) === true) {
        ...
        if (Wo_IsPostVoted($post_id, $wo['user']['user_id'])) {
            ...
            if ($vote) {
				...
				/* plugin points */
				if(!empty($wo['plugin_list']['plugin_actived']) && in_array('Points', $wo['plugin_list']['plugin_actived']) && !empty($wo['system']['userpoints_enable']) && $wo['system']['userpoints_enable']){ update_points( $wo['user']['user_id'], 'votepoll' ); }
				/* plugin points */
            }
        }
    }
    ...
    if (Wo_CheckSession($hash_id) === true) {
        ...
        if (empty($error)) {
            $registration_data = array(
                ...
                'content' => Wo_Secure($_POST['blog_content']),
            ...
            if ($last_id && is_numeric($last_id)) {
                if (!empty($_FILES["thumbnail"]["tmp_name"])) {
                    $fileInfo      = array(
                        ...
                        'crop' => array('width' => 300, 'height' => 190)
                    );
                    $media         = Wo_ShareFile($fileInfo);
                    $mediaFilename = $media['filename'];
